...
class std_logic():
    ...
    def set_value(self, value):
        ...
        if value == 1 or value == "1":
            ...
            output_string = output_string + "Send ^z\nsleep,50\n"
        else:
...
#testing pins
DisplayControl = std_logic(41, "DisplayControl", 0)
RegAControl = std_logic(24, "RegAControl", 0)
RegBControl = std_logic(25, "RegBControl", 0)
...
def main():
    ...
    #test the output register
    Test_Pattern = "10101111"
    for i in range(len(Main_Bus)):
        Main_Bus[i].set_value(int(Test_Pattern[-(i+1)]))
    MemWriteControl.set_value(0)
    MemWriteControl.set_value(1)
    Bus_Signal = "ZZZZZZZZ"
    for i in range(len(Main_Bus)):
        Main_Bus[i].set_value((Bus_Signal[-(i+1)]))
    MemOutEnable.set_value(0) #this should show whats in the memory

    #this bad boy is good to go now
    with open("test.ahk", "w") as output_file:
            output_file.write("^p::\n")
